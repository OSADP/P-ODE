...
public class PublicationRegistrationService {
    ...
    private static final int INCIDENT_MASK = Integer.parseInt("10000000", 2);
...
//            response.setServiceRegion(null);
    ...
    private PodeDataDestination populateDestinations(PodeDataTypes regData){
        ...
        byte[] value = regData.getValue().getValue();
        if(value.length == 1){
            ...
            if ((i & INCIDENT_MASK) == INCIDENT_MASK) {
                String messageType = ODEMessageType.INCIDENT.dataSource();
                QueueInfo queueInfo = getQueueForMessageType(messageType);

                destination.setIncidentURL(queueInfo.getWsHost() + queueInfo.getWsURL());
            }
        }
        return destination;
    }
