...
public class RegistrationDAOImpl implements RegistrationDAO {
    ...
    public ServiceRequest getServiceRequestForRequestId(byte[] requestId){
        ...
        try {
            ...
            GroupID groupId = new GroupID(ByteUtils.convertHexToBytes(row.getGroupId()));
            request.setGroupID(groupId);
            
            
            request.setRequestID(ByteUtils.convertHexToBytes(row.getRequestId()));
            
            PodeDialogID did = new PodeDialogID();
            did.setValue(PodeDialogID.EnumType.podeDataSubscriptionRegistration);
            request.setDialogID(did);
    ...
    public void updateRegistration(PodeSubscriptionRequest subRegistration, DataSubscriptionResponse subResponse){
        ...
        byte[] bytes = subRegistration.getSubData().getDataElements().getValue().getValue();
        if(bytes.length > 0){
    ...
    public void updateRegistration(PodeDataPulicationRegistration pubRegistration){
        ...
        byte[] bytes = pubRegistration.getRegData().getValue().getValue();
        if(bytes.length > 0){
            row.setDataTypes(Integer.toString(bytes[0], 2));
        }
